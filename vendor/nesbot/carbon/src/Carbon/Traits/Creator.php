...
trait Creator
{
    ...
    public function __construct($time = null, $tz = null)
    {
        ...
        // Kiểm tra giá trị trả về của parent::getLastErrors() trước khi gọi setLastErrors()
        $lastErrors = parent::getLastErrors();
        if (is_array($lastErrors)) {
            static::setLastErrors($lastErrors);
        } else {
            // Xử lý lỗi hoặc cung cấp giá trị mặc định cho $lastErrors ở đây nếu cần thiết
    ...
    // Kiểm tra xem $lastErrors có phải là mảng không trước khi gọi setLastErrors()
    if (is_array($lastErrors)) {
        $instance::setLastErrors($lastErrors);
    } else {
        // Xử lý khi $lastErrors không phải là mảng
        // Ví dụ: ghi log hoặc xử lý lỗi khác
    }
